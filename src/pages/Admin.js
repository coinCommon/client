...
import {createServices, fetchServices} from "../http/servicesAPI";
...
const Admin = () => {
    ...
    const [saveApiAndId, setSaveApiAndId] = useState({api: null, id: 0})
    // Загрузка данных при клике на кнопку 'редактировать'
    const getDataForEditing = (fetch, edit, title, id) => {
        if (id === 0) {alert(id + ' Непредвиденная ошибка!')}
            ...
            setSaveApiAndId({api: edit, id: id})
        }
    }
    ...
    return (
        <section className={classes.section_admin}>
            ...
            <EditModals edit={edit} setEdit={setEdit} dataEdit={dataEdit} setDataEdit={setDataEdit} SaveData={SaveData}/>
